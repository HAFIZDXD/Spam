...
try:
	os.system("clear")
	logo()
	print(f"{W}[{R}!{W}] Contoh Nomor HP{R}:{Y} 8531xxxxxxx")
	nomor=input(f"{W}[{R}?{W}] masukan nomor{R}:{G} ")
	jum=input(f"{W}[{R}?{W}] Jumlah spam {R}({biru}batas:5{R}){R}:{G} ")
	...
	if jum == "1":
		...
		wa()
		wa()
		print (f"{W}Berhasil Mengirim Spam {G}✓")
	if jum == "2":
		load()
		print()
		wa()
	...
	if jum == "5":
		...
		print (f"{W}Berhasil Mengirim spam {G}✓")
	if nomor == "" or nomor == " ":
		exit(f"{W}Masukin Yang Bener Kontol {R}! ! !{W}")
	if jum == "" or jum == " ":
		exit(f"{W}Masukin Yang Bener Kontol {R}! ! !{W}")
	if jum > "5":
